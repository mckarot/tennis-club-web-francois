...
// Nettoyage de l'URL si elle contient /_/
...
async function createEventsCollection() {
    try {
        console.log('Tentative de connexion à:', pbUrl);
        ...
        try {
            ...
            if (err.status === 400) {
                ...
                console.error('Erreur lors de la création/mise à jour de la collection:', err.message, err.data);
                throw err;
            }
        }
        ...
        console.error('Erreur fatale:', error.message, error.data);
    }
}
