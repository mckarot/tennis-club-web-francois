...
-- Vérifier le nombre de politiques RLS créées
SELECT 'Politiques RLS créées: ' || COUNT(*) as resultat
FROM pg_policies WHERE schemaname = 'public';

-- Vérifier que les tables ont RLS activé
SELECT 'Tables avec RLS: ' || COUNT(*) as resultat
FROM pg_tables WHERE schemaname = 'public' AND rowsecurity = true;

-- Vérifier les utilisateurs
SELECT 'Utilisateurs (public.users): ' || COUNT(*) as resultat
FROM public.users;

-- Vérifier les profils
SELECT 'Profils (public.profiles): ' || COUNT(*) as resultat
FROM public.profiles;

-- Vérifier les courts
SELECT 'Courts (public.courts): ' || COUNT(*) as resultat
FROM public.courts;
...
-- 2. Tester la connexion avec les 3 comptes :
